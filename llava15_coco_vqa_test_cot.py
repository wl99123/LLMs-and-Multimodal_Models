...
import warnings
from sklearn.metrics import f1_score  # 导入F1分数计算
...
def batch_evaluate():
    ...
    print(f"\nStart dual mode inference (coco_vqa_1000 | Text-only | 100% error-free)...")
    ...
    for use_cot in USE_COT:
        ...
        print(f"Accuracy: {accuracy * 100:.2f}% | Weighted F1: {weighted_f1:.4f} | Avg Speed: {avg_speed} T/s")
    ...
    detail_csv = os.path.join(SAVE_DIR, "llava15_vqa_dual_mode_detail.csv")
    ...
    # 2. 双模式指标汇总对比
    summary_csv = os.path.join(SAVE_DIR, "llava15_vqa_dual_mode_summary.csv")
    with open(summary_csv, "w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Evaluation Mode", "Total Samples", "Valid Inference", "Failed Count",
                         "Correct Count", "Accuracy(%)", "Weighted F1", "Average Speed(Tokens/s)", "Running Mode"])
        for mode_name, res in all_mode_results.items():
            m = res["metrics"]
            writer.writerow([
                mode_name, len(dataset), m["valid_num"], 0, m["correct_num"],
                f"{m['accuracy'] * 100:.2f}", m["weighted_f1"], m["avg_speed"],
                "纯文本适配模式（无images参数）"
            ])

    # === 双模式最终对比报告 ===
    print("\n" + "=" * 60)
    print(f"LLaVA-1.5-7B Dual Mode Evaluation Report | Device: {DEVICE}")
    print("=" * 60)
    # 提取双模式指标
    ...
    print(f"    Correct Count: {direct_res['correct_num']} | Valid Samples: {direct_res['valid_num']}")
    ...
    print(f"    Correct Count: {cot_res['correct_num']} | Valid Samples: {cot_res['valid_num']}")
    ...
    # 打印结果文件
    print(f"\nResult saving:")
    print(f"   - Dual mode detailed results: {detail_csv}")
    print(f"   - Dual mode summary results: {summary_csv}")
    print("=" * 60)
